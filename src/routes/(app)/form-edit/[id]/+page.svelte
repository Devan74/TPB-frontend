<script>
  ...
  const newField = {
    ...
    validation: {}, // Initialize an empty validation object
    advanced: {}, // Initialize an empty advanced object
  };
  ...
  async function updateForm() {
    try {
      ...
      const response = await fetch(`http://localhost:8000/api/forms/${data.form.id}`, {
        method: 'PUT',
        headers: {
  ...
  function updateAdvancedProperties(field, key, value) {
    const advanced = { ...field.advanced, [key]: value };
    updateField(field, 'advanced', advanced);
  }
</script>
                ...
                <div class="space-y-2">
                  <label class="flex items-center gap-2">
                    <input
                      type="checkbox"
                      class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500"
                      bind:checked={selectedField.advanced.readonly}
                      on:change={() => updateAdvancedProperties(selectedField, 'readonly', selectedField.advanced.readonly)}
                    />
                  ...
                  <label class="flex items-center gap-2">
                    <input
                      type="checkbox"
                      class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500"
                      bind:checked={selectedField.advanced.disabled}
                      on:change={() => updateAdvancedProperties(selectedField, 'disabled', selectedField.advanced.disabled)}
                    />
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-indigo-500 focus:border-indigo-500"
                      bind:value={selectedField.advanced.customClasses}
                      on:input={() => updateAdvancedProperties(selectedField, 'customClasses', selectedField.advanced.customClasses)}
